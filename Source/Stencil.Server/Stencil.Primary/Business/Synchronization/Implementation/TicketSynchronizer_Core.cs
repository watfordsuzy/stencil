...
{
    public partial class TicketSynchronizer : SynchronizerBase<Guid>, ITicketSynchronizer
    {
        public TicketSynchronizer(IFoundation foundation)
            : base(foundation, "TicketSynchronizer")
        {
        ...
        public override void PerformSynchronizationForItem(Guid primaryKey)
        {
            ...
            {
                Ticket domainModel = this.API.Direct.Tickets.GetById(primaryKey);
                if (domainModel != null)
                {
                    Action<Guid, bool, DateTime, string> synchronizationUpdateMethod = this.API.Direct.Tickets.SynchronizationUpdate;
                    if(this.API.Integration.SettingsResolver.IsHydrate())
                    {
                        synchronizationUpdateMethod = this.API.Direct.Tickets.SynchronizationHydrateUpdate;
                    }
                    ...
                    try
                    {
                        sdk.Ticket sdkModel = domainModel.ToSDKModel();
                        ...
                        {
                            this.API.Index.Tickets.DeleteDocument(sdkModel);
                            synchronizationUpdateMethod(domainModel.ticket_id, true, syncDate, null);
                        }
                        else
                        {
                            IndexResult result = this.API.Index.Tickets.UpdateDocument(sdkModel);
                            if (result.success)
                            {
                                synchronizationUpdateMethod(domainModel.ticket_id, true, syncDate, result.ToString());
                            }
                            else
                            {
        ...
        public override int PerformSynchronization(string requestedAgentName)
        {
            return base.ExecuteFunction("PerformSynchronization", delegate ()
            {
                ...
                if(this.API.Integration.SettingsResolver.IsHydrate())
                {
                    invalidItems = this.API.Direct.Tickets.SynchronizationHydrateGetInvalid(CommonAssumptions.INDEX_RETRY_THRESHOLD_SECONDS, agentName);
                }
                else
                {
                    invalidItems = this.API.Direct.Tickets.SynchronizationGetInvalid(CommonAssumptions.INDEX_RETRY_THRESHOLD_SECONDS, agentName);
                }
        ...
        /// </summary>
        protected void HydrateSDKModelComputed(Ticket domainModel, sdk.Ticket sdkModel)
        {
            
        }
        partial void HydrateSDKModel(Ticket domainModel, sdk.Ticket sdkModel);
    }
}
