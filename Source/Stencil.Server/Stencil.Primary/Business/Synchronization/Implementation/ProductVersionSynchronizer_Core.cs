...
{
    public partial class ProductVersionSynchronizer : SynchronizerBase<Guid>, IProductVersionSynchronizer
    {
        public ProductVersionSynchronizer(IFoundation foundation)
            : base(foundation, "ProductVersionSynchronizer")
        {
        ...
        public override void PerformSynchronizationForItem(Guid primaryKey)
        {
            ...
            {
                ProductVersion domainModel = this.API.Direct.ProductVersions.GetById(primaryKey);
                if (domainModel != null)
                {
                    Action<Guid, bool, DateTime, string> synchronizationUpdateMethod = this.API.Direct.ProductVersions.SynchronizationUpdate;
                    if(this.API.Integration.SettingsResolver.IsHydrate())
                    {
                        synchronizationUpdateMethod = this.API.Direct.ProductVersions.SynchronizationHydrateUpdate;
                    }
                    ...
                    try
                    {
                        sdk.ProductVersion sdkModel = domainModel.ToSDKModel();
                        ...
                        {
                            this.API.Index.ProductVersions.DeleteDocument(sdkModel);
                            synchronizationUpdateMethod(domainModel.product_version_id, true, syncDate, null);
                        }
                        else
                        {
                            IndexResult result = this.API.Index.ProductVersions.UpdateDocument(sdkModel);
                            if (result.success)
                            {
                                synchronizationUpdateMethod(domainModel.product_version_id, true, syncDate, result.ToString());
                            }
                            else
                            {
        ...
        public override int PerformSynchronization(string requestedAgentName)
        {
            return base.ExecuteFunction("PerformSynchronization", delegate ()
            {
                ...
                if(this.API.Integration.SettingsResolver.IsHydrate())
                {
                    invalidItems = this.API.Direct.ProductVersions.SynchronizationHydrateGetInvalid(CommonAssumptions.INDEX_RETRY_THRESHOLD_SECONDS, agentName);
                }
                else
                {
                    invalidItems = this.API.Direct.ProductVersions.SynchronizationGetInvalid(CommonAssumptions.INDEX_RETRY_THRESHOLD_SECONDS, agentName);
                }
        ...
        partial void HydrateSDKModel(ProductVersion domainModel, sdk.ProductVersion sdkModel);
    }
}
